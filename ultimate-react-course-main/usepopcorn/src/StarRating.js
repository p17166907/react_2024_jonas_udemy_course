...
export function StarRating({ maxRating = 5, color = "#fcc419", size = 48, className = "", messages = [], defaultRating = 0, setMovieRating }) {
  ...
  const handleRating = (rating, i) => { setRating(rating + i); setMovieRating(rating + i) }
  ...
  return (
    <div style={containerStyle} className={className}>
      <div style={starContainerStyle}> {Array.from({ length: maxRating }, (_, i) => (
        <Star key={i} onRateClick={() => { handleRating(1, i) }} full={isFull(rating, tempRating, i)} handleOnHoverIn={() => { handleOnHoverIn(i) }}
          handleOnHoverOut={handleOnHoverOut} color={color} size={size} />))}
      </div>
