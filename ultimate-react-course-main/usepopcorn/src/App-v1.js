...
export default function App() {
	...
	return (
		<>
			<NavBar movies={movies} > <Search /> <NumResults movies={movies} /> </NavBar>
			<Main>
				<Box >
...
export function MovieList({ movies, setMovies }) {
	return (<ul className="list"> {movies?.map((movie, index) => (<Movie key={movie.imdbID} movie={movie} />))} </ul>)
}
...
/*
export function WatchedBox({ isOpen2, setIsOpen2, watched }) {
	return (
		<div className="box">
			<button className="btn-toggle" onClick={() => setIsOpen2(!isOpen2)}>	{isOpen2 ? "–" : "+"}	</button>
			{isOpen2 && (<> <WatchedSummary watched={watched} />	<WatchedMovieList watched={watched} /> </>)}
		</div>
	);
}
*/
...
export function WatchedMovieList({ watched }) { return (<ul className="list">	{watched.map(movie => (<WatchedMovie key={movie.imdbID} movie={movie} />))}	</ul>); }
